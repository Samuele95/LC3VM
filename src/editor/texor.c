...
static char updatedfname[PATH_MAX];
...
int mount_drive_volume() {
    ...
    if (!mkdir((get_working_dir(), "/utils/"), 0777) && errno == EEXIST) {
        printf("No need to create volume in %s\n", cwd);
        return EXIT_FAILURE;
    }
    strcat(cwd,"/utils/");
...
void editorSave() {
  ...
  strcpy(updatedfname, E.filename);
  if (strstr(updatedfname, cwd) == NULL) {
      memset(updatedfname,0, strlen(updatedfname));
      strcpy(updatedfname, cwd);
      strcat(updatedfname, E.filename);
  }
  int fd = open(updatedfname, O_RDWR | O_CREAT, 0644);
  if (fd != -1) {
    ...
    if (filerow >= E.number_of_rows ) {
      if (E.number_of_rows == 0 && y == E.screen_rows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
            "LC3 Program editor using Texor editor -- version %s\n", TEXOR_VERSION);
...
void editorProcessKeypress() {
  ...
  switch(c) {
    ...
    case CTRL_KEY('a'):
      editorSave();
      strcpy(command, "./lc3compiler ");
      strcat(command, updatedfname);
      if ((status = WEXITSTATUS(system(command))) != 0)
          editorSetStatusMessage("Program didn't compile. Exit status: %d. "
          "Check log files", status);
      else
          editorSetStatusMessage("Program compiled successfully!",status);
      break;
...
int main(int argc, char *argv[]) {
  ...
  editorSetStatusMessage(
      "HELP: Ctrl-S = save | Ctrl-Q = quit | Ctrl-F = find | Ctrl-A = compile");
